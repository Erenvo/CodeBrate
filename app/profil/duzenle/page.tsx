...
export default function ProfilDuzenle() {
  ...
  const [formData, setFormData] = useState({
    username: '',   // NICKNAME (Örn: erenvo)
    full_name: '',  // AD SOYAD (Örn: Eren Alpaslan)
    university: '',
    department: '',
    bio: '',
    github_link: '',
    ...
    const getProfile = async () => {
      ...
      if (data) {
        setFormData({
          ...
          full_name: data.full_name || '', // <-- AD SOYAD BURADAN GELİYOR
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    if (!formData.username || formData.username.trim() === '') {
      setMessage('Hata: Kullanıcı adı boş bırakılamaz.')
      return
    }
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-8 shadow-lg">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                ...
                <input 
                  ...
                  className="w-full rounded-md bg-gray-900 border border-gray-700 px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                  placeholder="Adınız Soyadınız" 
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Kullanıcı Adı (Giriş için) *</label>
                <input 
                  type="text" 
                  value={formData.username} 
                  onChange={(e) => setFormData({ ...formData, username: e.target.value.toLowerCase().replace(/\s/g, '') })} 
                  className={`w-full rounded-md bg-gray-900 border px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none ${
                    !formData.username ? 'border-red-500' : 'border-gray-700'
                  }`}
                  placeholder="nickname" 
                  required
                />
                {!formData.username && (
                  <p className="text-sm text-red-400 mt-1">✗ Kullanıcı adı boş bırakılamaz</p>
                )}
              </div>
            </div>
            ...
            <div className="pt-4">
              <button type="submit" disabled={isSaving || !formData.username} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none disabled:opacity-50">{isSaving ? 'Kaydediliyor...' : 'Değişiklikleri Kaydet'}</button>
