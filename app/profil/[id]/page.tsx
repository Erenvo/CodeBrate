...
export default function GenelProfilSayfasi() {
  ...
  const [projects, setProjects] = useState<ProjectType[]>([]) // Kullanıcının projeleri
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (profileError) {
        ...
        // 2. Bu Kullanıcının Projelerini Çek (Sadece profil bulunduy­sa)
        const { data: projectsData, error: projectsError } = await supabase
          .from('projects')
          .select('id, title, showcase_description, category_tags, created_at')
          .eq('owner_id', id) // Sadece bu ID'ye ait olanlar
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        {/* ÜST KISIM: Profil Başlık Kartı (Değişmedi) */}
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-4 text-sm text-gray-400">
                {profile.university && <div className="flex items-center gap-1"><MapPin size={16} className="text-indigo-400" /><span>{profile.university}</span></div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* SAĞ KOLON: Projeler Listesi (GÜNCELLENDİ) */}
          <div className="md:col-span-2">
            ...
            {projects.length > 0 ? (
              ...
              // Proje Yoksa Gösterilecek Kısım
